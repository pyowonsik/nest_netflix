...
describe('MovieService', () => {
  ...
  describe('find One', () => {
    let findMovieDetail: jest.SpyInstance;

    beforeEach(() => {
      findMovieDetail = jest.spyOn(movieService, 'findMovieDetail');
    });
    ...
    it('should reutrn a movie if found', async () => {
      ...
      findMovieDetail.mockResolvedValue(movie);

      const result = await movieService.findOne(movie.id);

      // mocking한 함수는 findOne이 아닌, mockResolvedValue
      expect(findMovieDetail).toHaveBeenCalledWith(movie.id);
      expect(result).toEqual(movie);
    });

    it('should throw NotFoundException if movie is not found', async () => {
      findMovieDetail.mockResolvedValue(null);

      // result = await movieService.findOne(1)

      await expect(movieService.findOne(1)).rejects.toThrow(NotFoundException);
      expect(findMovieDetail).toHaveBeenCalledWith(1);
    });
  });
